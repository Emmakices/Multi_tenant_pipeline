...
def process_ecommerce_data(file_path: str, file_name: str, tenant_id: str, region: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    try:
        ...
        logger.info(f"Final clean data shape: {final_df.shape}, Bad records: {len(bad_records)}")
...
def validate_schema_dask(df: dd.DataFrame) -> Tuple[dd.DataFrame, pd.DataFrame]:
    ...
    if missing_cols:
        ...
        return dd.from_pandas(pd.DataFrame(), npartitions=1), df.compute()  # All records are bad if schema is wrong
...
def perform_data_quality_checks_dask(df: dd.DataFrame) -> Tuple[dd.DataFrame, pd.DataFrame]:
    ...
    try:
        ...
        df_deduplicated = df_with_index.drop_duplicates(subset=['user_id', 'product_id', 'event_time'], keep='first')
...
def create_ml_features_dask(df: dd.DataFrame, tenant_id: str, file_name: str, region: str) -> dd.DataFrame:
...
def save_to_bigquery_historical_dask(df: pd.DataFrame, file_name: str, region: str, tenant_id: str) -> int:
...
def save_bad_records_dask(bad_df: pd.DataFrame, bucket_name: str, file_name: str, tenant_id: str):
    ...
    try:
        ...
        if tenant_id == 'shared':
            bad_records_path = f"shared/bad_records/dask_failed_{timestamp}_{original_filename}.csv"
        else:
...
def move_file_to_bad_records_dask(bucket_name: str, file_name: str, error_message: str, tenant_id: str):
    ...
    try:
        ...
        if tenant_id == 'shared':
            bad_records_path = f"shared/bad_records/dask_processing_failed_{timestamp}_{filename}"
        else:
...
def process_file():
    ...
    try:
        ...
        if not processed_data.empty:
            good_record_count = save_to_bigquery_historical_dask(processed_data, file_name, region, tenant_id)
        ...
        if 'file_name' in locals() and 'bucket_name' in locals():
            move_file_to_bad_records_dask(bucket_name, file_name, str(e), tenant_id if 'tenant_id' in locals() else 'unknown')

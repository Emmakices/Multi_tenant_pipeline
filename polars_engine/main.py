...
def process_ecommerce_data(file_path: str, file_name: str, tenant_id: str, region: str) -> Tuple[pl.DataFrame, pl.DataFrame]:
    """
    ...
    try:
        ...
        if schema_bad.height > 0:
            schema_bad = schema_bad.with_columns(pl.lit('schema_validation_failed').alias('bad_reason'))
        ...
        logger.info(f"Final clean data shape: {df.shape}, Bad records: {bad_records.height}")
...
def validate_schema_polars(df: pl.DataFrame) -> Tuple[pl.DataFrame, pl.DataFrame]:
    ...
    try:
        ...
        clean_df = clean_df.with_columns([
            ...
            pl.col('event_time').str.strptime(pl.Datetime, format='%Y-%m-%d %H:%M:%S UTC', strict=False).alias('event_time_parsed')
        ])
        ...
        if null_count > 0:
            clean_df = clean_df.with_columns([
                ...
                .then(pl.col('event_time').str.strptime(pl.Datetime, format='%Y-%m-%d %H:%M:%S', strict=False))
                .otherwise(pl.col('event_time_parsed'))
                .alias('event_time')
            ]).drop('event_time_parsed')
        else:
            clean_df = clean_df.with_columns(pl.col('event_time_parsed').alias('event_time')).drop('event_time_parsed')
...
def perform_data_quality_checks_polars(df: pl.DataFrame) -> Tuple[pl.DataFrame, pl.DataFrame]:
    ...
    try:
        ...
        if bad_records.height > 0:
            bad_records = bad_records.with_columns(pl.lit('data_quality_failed').alias('bad_reason'))
...
def create_ml_features_polars(df: pl.DataFrame, tenant_id: str, file_name: str, region: str) -> pl.DataFrame:
    ...
    try:
        ...
        session_stats = df.group_by('user_session').agg([
        ...
        unique_categories = df.get_column('category_code').fill_null('unknown').unique().to_list()
        ...
        user_conversion = user_total_events.join(user_purchases, on='user_id', how='left').with_columns([
            (pl.col('user_purchases').fill_null(0) / pl.col('user_total_events_for_conversion'))
            .alias('user_conversion_rate')
        ])
        
        df = df.join(user_conversion.select(['user_id', 'user_conversion_rate']), on='user_id', how='left')
...
def save_to_bigquery_historical_polars(df: pl.DataFrame, file_name: str, region: str, tenant_id: str) -> int:
    ...
    try:
        ...
        job = bq_client.load_table_from_dataframe(pandas_df, table_ref, job_config=job_config)
...
def save_bad_records_polars(bad_df: pl.DataFrame, bucket_name: str, file_name: str, tenant_id: str):
    ...
    try:
        ...
        if tenant_id == 'shared':
            bad_records_path = f"shared/bad_records/polars_failed_{timestamp}_{original_filename}.csv"
        else:
...
def move_file_to_bad_records_polars(bucket_name: str, file_name: str, error_message: str, tenant_id: str):
    ...
    try:
        ...
        if tenant_id == 'shared':
            bad_records_path = f"shared/bad_records/polars_processing_failed_{timestamp}_{filename}"
        else:
...
def process_file():
    ...
    try:
        ...
        if processed_data.height > 0:
            good_record_count = save_to_bigquery_historical_polars(processed_data, file_name, region, tenant_id)
        ...
        if 'file_name' in locals() and 'bucket_name' in locals():
            move_file_to_bad_records_polars(bucket_name, file_name, str(e), tenant_id if 'tenant_id' in locals() else 'unknown')

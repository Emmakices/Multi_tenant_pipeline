...
class TestAuthToken:
    ...
    def test_get_auth_token_success(self, mock_auth_token):
        ...
        with patch('google.auth.transport.requests.Request') as mock_request, \
             patch('google.oauth2.id_token.fetch_id_token', return_value=mock_auth_token) as mock_fetch:
    ...
    def test_get_auth_token_failure(self):
        ...
        with patch('google.auth.transport.requests.Request') as mock_request, \
             patch('google.oauth2.id_token.fetch_id_token', side_effect=Exception("Auth failed")) as mock_fetch, \
    ...
    def test_get_auth_token_various_exceptions(self, exception_type, exception_msg):
        ...
        with patch('google.auth.transport.requests.Request'), \
             patch('google.oauth2.id_token.fetch_id_token', side_effect=exception_type(exception_msg)), \
             patch('builtins.print') as mock_print:
...
class TestEngineHealth:
    ...
    def test_engine_health_success_with_auth(self, healthy_engine_response, mock_auth_token):
        """Test successful engine health check with authentication."""
        engine_name = "pandas"
        engine_url = "https://pandas-test.run.app"
        
        with patch('main.get_auth_token', return_value=mock_auth_token), \
    ...
    def test_engine_health_success_without_auth(self, healthy_engine_response):
        ...
        with patch('main.get_auth_token', return_value=None), \
             patch('requests.get') as mock_get, \
    ...
    def test_engine_health_connection_error(self):
        ...
        with patch('main.get_auth_token', return_value=None), \
             patch('requests.get', side_effect=requests.exceptions.ConnectionError(connection_error)), \
             patch('builtins.print') as mock_print:
            ...
            assert any("CONNECTION_ERROR polars: Connection Error" in str(call) 
                      for call in mock_print.call_args_list)
    ...
    def test_engine_health_unexpected_error(self):
        ...
        with patch('main.get_auth_token', return_value=None), \
             patch('requests.get', side_effect=Exception(unexpected_error)), \
             patch('builtins.print') as mock_print:
...
class TestMainFunction:
    ...
    def test_main_all_engines_healthy(self, healthy_engine_response):
        ...
        with patch('main.check_engine_health') as mock_health_test, \
             patch('builtins.print') as mock_print:
    ...
    def test_main_some_engines_unhealthy(self):
        ...
        with patch('main.check_engine_health', side_effect=mock_health_response), \
             patch('builtins.print') as mock_print:
...
class TestIntegration:
    ...
    def test_complete_workflow_success(self, mock_auth_token, healthy_engine_response):
        ...
        with patch('google.auth.transport.requests.Request') as mock_request, \
             patch('google.oauth2.id_token.fetch_id_token', return_value=mock_auth_token), \
             patch('requests.get') as mock_get, \
             patch('builtins.print') as mock_print:
    ...
    def test_mixed_auth_scenarios(self, mock_auth_token, healthy_engine_response):
        ...
        with patch('main.get_auth_token', side_effect=mock_auth_response), \
             patch('requests.get') as mock_get, \
             patch('builtins.print'):
...
class TestEdgeCases:
    ...
    def test_malformed_engine_urls(self):
        ...
        with patch('main.ENGINES', bad_engines), \
             patch('main.get_auth_token', return_value=None), \
...
class TestParametrized:
    ...
    @pytest.mark.parametrize("engine_name,expected_url", [
        ("pandas", ENGINES["pandas"]),
        ("polars", ENGINES["polars"]),
        ("dask", ENGINES["dask"])
    ])
    def test_each_engine_individually(self, engine_name, expected_url, healthy_engine_response):
        """Test each engine individually with parameterized inputs."""
        with patch('main.get_auth_token', return_value="test-token"), \
             patch('requests.get') as mock_get, \
             patch('builtins.print'):
    ...
    @pytest.mark.parametrize("error_type,error_message,expected_status", [
        (requests.exceptions.Timeout(), "Request timeout", "timeout"),
        (requests.exceptions.ConnectionError("Connection failed"), "Connection failed", "connection_error"),
        (requests.exceptions.HTTPError("HTTP Error"), "HTTP Error", "error"),
        (requests.exceptions.RequestException("Request failed"), "Request failed", "error"),
        (Exception("Generic error"), "Generic error", "error")
    ])
    def test_various_request_exceptions(self, error_type, error_message, expected_status):
        """Test various request exceptions with parameterized inputs."""
        with patch('main.get_auth_token', return_value=None), \
             patch('requests.get', side_effect=error_type), \
             patch('builtins.print'):

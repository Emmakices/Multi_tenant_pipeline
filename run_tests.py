...
class TestRunner:
    ...
    def run_engine_tests(self, engine: str, test_args: List[str] = None) -> Dict:
        ...
        try:
            ...
            logger.warning(
                f"pytest-xdist not available for {engine}, running sequentially"
            )
    ...
    def run_parallel_tests(
        ...
        logger.info(
            f"Running tests in parallel across {len(engines)} engines with {max_workers} workers"
        )
        start_time = time.time()
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(future_to_engine):
                ...
                try:
                    ...
                    logger.info(
                        f"[{status_symbol}] {engine}: {result['status']} in {result['duration']:.1f}s "
    ...
    def run_sequential_tests(
        self, engines: List[str] = None, test_args: List[str] = None
    ) -> Dict:
    ...
    def print_summary(self, results: Dict):
        ...
        for engine, result in results["engines"].items():
            ...
            print(
                f"[{status_symbol:7}] {engine:15} | {status:8} | {duration:6.1f}s | "
...
def main():
    ...
    parser.add_argument(
        "--save-results", default="test_results.json", help="Save results to JSON file"
    )
    parser.add_argument(
        "--fast", action="store_true", help="Run only fast tests (< 5 seconds)"
    )
    parser.add_argument(
        "--coverage",
        action="store_true",

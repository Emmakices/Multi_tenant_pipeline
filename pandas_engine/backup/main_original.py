...
def process_file():
    ...
    try:
        ...
        logger.info(f"Processing {file_name} from {bucket_name}, Size: {file_size} bytes, Tenant: {tenant_id}")
        
        # Download file from GCS
        file_path = download_file_from_gcs(bucket_name, file_name)
        
        # Process file with ML-ready transformations
        processed_data, bad_records = process_ecommerce_data(file_path, file_name, tenant_id)
        
        # Save good records to BigQuery (Historical append mode)
        good_record_count = 0
        if not processed_data.empty:
            good_record_count = save_to_bigquery_historical(processed_data, file_name, region, tenant_id)
        ...
        if 'file_name' in locals() and 'bucket_name' in locals():
            move_file_to_bad_records(bucket_name, file_name, str(e), tenant_id if 'tenant_id' in locals() else 'unknown')
...
def process_ecommerce_data(file_path: str, file_name: str, tenant_id: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    ...
    try:
        ...
        if not df.empty:
            df = create_ml_features(df, tenant_id, file_name)
        
        logger.info(f"Final clean data shape: {df.shape}, Bad records: {len(bad_records)}")
...
def perform_data_quality_checks(df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if 'price' in df.columns and not df['price'].isna().all():
        ...
        if price_std > 0:
            price_outliers = (df['price'] > price_mean + 3 * price_std) | (df['price'] < 0)
    ...
    duplicates = df.duplicated(subset=['user_id', 'product_id', 'event_time'], keep='first')
    bad_mask |= duplicates
    ...
    if bad_mask.any():
        ...
        for i, is_bad in enumerate(bad_mask):
            if is_bad:
                ...
                if df.iloc[i]['event_type'] == 'purchase' and (pd.isna(df.iloc[i]['price']) or df.iloc[i]['price'] <= 0):
                    reasons.append('invalid_purchase_price')
                bad_records.loc[bad_records.index[sum(bad_mask[:i+1])-1], 'bad_reason'] = ','.join(reasons) if reasons else 'data_quality_failed'
    
    clean_df = df[~bad_mask].copy()
    
    return clean_df, bad_records

def create_ml_features(df: pd.DataFrame, tenant_id: str, file_name: str) -> pd.DataFrame:
    ...
    try:
        ...
        df['region'] = region  # Add region tracking
        ...
        df['time_since_last_event'] = df.groupby('user_id')['event_time'].diff().dt.total_seconds()
        ...
        session_stats.columns = ['user_session'] + ['_'.join(col).strip() for col in session_stats.columns[1:]]
        ...
        df = df.merge(session_stats[['user_session', 'session_event_count', 'session_unique_products', 
                                    'session_total_value', 'session_avg_price', 'session_max_price', 
                                    'session_duration_minutes']], on='user_session', how='left')
        ...
        df['category_code_encoded'] = df['category_code'].fillna('unknown').astype('category').cat.codes
        ...
        category_price_stats = df.groupby('category_id')['price'].agg(['mean', 'std']).reset_index()
        df = df.merge(category_price_stats, on='category_id', how='left', suffixes=('', '_category'))
        ...
        user_stats.columns = ['user_id', 'user_total_events', 'user_unique_products', 
...
def save_to_bigquery_historical(df: pd.DataFrame, file_name: str, region: str, tenant_id: str) -> int:
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Using existing dataset: {dataset_id} in {existing_dataset.location}")
        except:
        ...
        schema = [
            ...
            bigquery.SchemaField("tenant_id", "STRING", mode="REQUIRED", description="Tenant identifier for data isolation"),
            bigquery.SchemaField("region", "STRING", mode="REQUIRED", description="Processing region"),
            
            # Original e-commerce data
            bigquery.SchemaField("event_time", "TIMESTAMP", mode="REQUIRED", description="Original event timestamp"),
            bigquery.SchemaField("event_type", "STRING", mode="REQUIRED", description="Type of e-commerce event"),
            bigquery.SchemaField("product_id", "INTEGER", mode="REQUIRED", description="Product identifier"),
            bigquery.SchemaField("category_id", "INTEGER", mode="NULLABLE", description="Product category identifier"),
            bigquery.SchemaField("category_code", "STRING", mode="NULLABLE", description="Product category code"),
            bigquery.SchemaField("brand", "STRING", mode="NULLABLE", description="Product brand"),
            bigquery.SchemaField("price", "FLOAT", mode="NULLABLE", description="Product price"),
            bigquery.SchemaField("user_id", "INTEGER", mode="REQUIRED", description="User identifier"),
            bigquery.SchemaField("user_session", "STRING", mode="NULLABLE", description="User session identifier"),
            
            # Processing metadata
            bigquery.SchemaField("original_filename", "STRING", mode="REQUIRED", description="Source file name"),
            bigquery.SchemaField("processed_timestamp", "TIMESTAMP", mode="REQUIRED", description="Data processing timestamp"),
            bigquery.SchemaField("processing_engine", "STRING", mode="REQUIRED", description="Processing engine used"),
            bigquery.SchemaField("data_quality_status", "STRING", mode="REQUIRED", description="Data quality validation status"),
            bigquery.SchemaField("created_at", "TIMESTAMP", mode="REQUIRED", description="Record creation timestamp"),
            
            # ML Features - Temporal
            bigquery.SchemaField("hour", "INTEGER", mode="NULLABLE", description="Hour of event"),
            bigquery.SchemaField("day_of_week", "INTEGER", mode="NULLABLE", description="Day of week (0=Monday)"),
            bigquery.SchemaField("month", "INTEGER", mode="NULLABLE", description="Month of event"),
            bigquery.SchemaField("is_weekend", "INTEGER", mode="NULLABLE", description="Weekend indicator (0/1)"),
            
            # ML Features - User behavior
            bigquery.SchemaField("time_since_last_event", "FLOAT", mode="NULLABLE", description="Seconds since user's last event"),
            bigquery.SchemaField("session_event_count", "INTEGER", mode="NULLABLE", description="Number of events in session"),
            bigquery.SchemaField("session_unique_products", "INTEGER", mode="NULLABLE", description="Unique products in session"),
            bigquery.SchemaField("session_total_value", "FLOAT", mode="NULLABLE", description="Total session value"),
            bigquery.SchemaField("session_avg_price", "FLOAT", mode="NULLABLE", description="Average price in session"),
            bigquery.SchemaField("session_max_price", "FLOAT", mode="NULLABLE", description="Maximum price in session"),
            bigquery.SchemaField("session_duration_minutes", "FLOAT", mode="NULLABLE", description="Session duration in minutes"),
            
            # ML Features - Product and user aggregations
            bigquery.SchemaField("product_popularity", "INTEGER", mode="NULLABLE", description="Product interaction count"),
            bigquery.SchemaField("price_vs_category_avg", "FLOAT", mode="NULLABLE", description="Price relative to category average"),
            bigquery.SchemaField("user_total_events", "INTEGER", mode="NULLABLE", description="User's total events in file"),
            bigquery.SchemaField("user_unique_products", "INTEGER", mode="NULLABLE", description="User's unique products in file"),
            bigquery.SchemaField("user_total_spent", "FLOAT", mode="NULLABLE", description="User's total spending in file"),
            bigquery.SchemaField("user_avg_price", "FLOAT", mode="NULLABLE", description="User's average price in file"),
            bigquery.SchemaField("user_unique_categories", "INTEGER", mode="NULLABLE", description="User's unique categories in file"),
            bigquery.SchemaField("user_conversion_rate", "FLOAT", mode="NULLABLE", description="User's conversion rate in file"),
            
            # ML Features - Encoded categories
            bigquery.SchemaField("brand_encoded", "INTEGER", mode="NULLABLE", description="Encoded brand category"),
            bigquery.SchemaField("category_code_encoded", "INTEGER", mode="NULLABLE", description="Encoded category code"),
        ...
        for col in event_columns:
            schema.append(bigquery.SchemaField(col, "INTEGER", mode="NULLABLE", description=f"One-hot encoded {col}"))
...
def save_bad_records(bad_df: pd.DataFrame, bucket_name: str, file_name: str, tenant_id: str):
    ...
    try:
        ...
        if tenant_id == 'shared':
            bad_records_path = f"shared/bad_records/pandas_failed_{timestamp}_{original_filename}.csv"
        else:
...
def move_file_to_bad_records(bucket_name: str, file_name: str, error_message: str, tenant_id: str):
    ...
    try:
        ...
        if tenant_id == 'shared':
            bad_records_path = f"shared/bad_records/pandas_processing_failed_{timestamp}_{filename}"
        else:
